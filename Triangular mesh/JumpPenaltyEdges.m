...
GeoTol = 1e-1; % the geometric tolerance that we regard an edge aligned with a barrier
...
for m = 1 : length(BarriersPath)
    ...
    if (BarriersStartRefCoord(m,1)+BarriersEndRefCoord(m,1)<GeoTol)&&(EtoEmap(1,jj)~=-1) % aligned with edge 1
        E = EtoEmap(1,jj);
        pp =  EtoEmap(:,E) == jj ;
        JumpPenaltyEdges(1,jj)=1;
        JumpPenaltyEdges(pp,E)=1;
    elseif (BarriersStartRefCoord(m,2)+BarriersEndRefCoord(m,2)<GeoTol)&&(EtoEmap(2,jj)~=-1)% aligned with edge 2
        E = EtoEmap(2,jj);
        pp =  EtoEmap(:,E) == jj ;
        JumpPenaltyEdges(2,jj)=1;
        JumpPenaltyEdges(pp,E)=1;
    elseif (BarriersStartRefCoord(m,3)+BarriersEndRefCoord(m,3)<GeoTol)&&(EtoEmap(3,jj)~=-1)% aligned with edge 3
        E = EtoEmap(3,jj);
        pp =  EtoEmap(:,E) == jj ;
