...
%FluxPenaltyEdges = JumpPenaltyEdges(coordinates,elements3,EtoEmap,Tarea,ParametersofBarrier,BarriersPath,BarriersLength);
FluxPenaltyEdges = JumpPenaltyEdges(EtoEmap,BarriersPath,BarriersStartRefCoord,BarriersEndRefCoord);

[HighOrderP,HighOrderUx,HighOrderUy,HighOrderSx,HighOrderSy] = EllipticLDGSolver;
% draw contour
...
function [HighOrderP,HighOrderUx,HighOrderUy,HighOrderSx,HighOrderSy] = EllipticLDGSolver
% define the variables
[HighOrderP,HighOrderUx,HighOrderUy,HighOrderSx,HighOrderSy] = deal( zeros( HighOrderNp , size(elements3,2) ) ); % unknowns 
[IMat,JMat] = ...
...
HighOrderSx(:) = A0\(B1*HighOrderP(:)+D1+Cp11*HighOrderUx(:)+Cp12*HighOrderUy(:));
HighOrderSy(:) = A0\(B2*HighOrderP(:)+D2+Cp21*HighOrderUx(:)+Cp22*HighOrderUy(:));
...
HighOrderSx(2:end,BarriersPath)=0;
HighOrderSy(2:end,BarriersPath)=0;
end
%--------------------------------------------------------------------------
function p = p(xy)
x = xy(:,1);
