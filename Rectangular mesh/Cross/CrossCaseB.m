function CrossCaseB % Case (b): Crossing Barrier.
...
alpha = 1/hx; % penalty on p jump to make the pressure continuous cross interfaces
beta = 5/hx; % penalty on u jump to make the flux continuous cross interfaces
...
ParametersofBarrier(:,1:4) = [0.5,0.5,0.5,0; 0.5,0.5,0.5,pi/2;]+(rand(NumberofBarriers,4))*1e-7;% position of barriers
ParametersofBarrier(:,5) = 1e-3; % width of fractures
ParametersofBarrier(:,6)= 1e-8 ; % tangential permeability_nu of fractures
ParametersofBarrier(:,7)= 1e-8 ; % normal permeability_nu of fractures
...
Gauss_num = 5 ; % the number of quadrature point in [-1,1] 
...
% HighOrderP( 2:end , BarriersPath(end-ParametersofBarrier(2,end)+1:end) )=0;
