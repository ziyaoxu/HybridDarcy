function RealCase % complex fracture network
% time: 2020.5.28 - 2020.6.6
% author : xuziyao
...
alpha = 0.0001/hx^2; % penalty on p jump to make the pressure continuous cross interfaces
...
FractureData(:,1)=[]; 
FractureData = (FractureData-300)*(1-1e-9)+300;
NumberofFractures = size(FractureData,1); NumberofBarriers = 0;
...
ParametersofFracture(:,5) = 1e-2; % width of fractures
ParametersofFracture(:,6)= 1e-8 ; % tangential permeability_nu of fractures
ParametersofFracture(:,7)= 1e-8 ; % normal permeability_nu of fractures
...
BasisPonCorners = zeros(HighOrderNp,4);
for i = 1 : HighOrderNp
    BasisPonCorners(i,1) = JacobiP(-1,0,0,x_index(i)-1)*JacobiP(-1,0,0,y_index(i)-1); 
    BasisPonCorners(i,2) = JacobiP(+1,0,0,x_index(i)-1)*JacobiP(-1,0,0,y_index(i)-1);
    BasisPonCorners(i,3) = JacobiP(+1,0,0,x_index(i)-1)*JacobiP(+1,0,0,y_index(i)-1);
    BasisPonCorners(i,4) = JacobiP(-1,0,0,x_index(i)-1)*JacobiP(+1,0,0,y_index(i)-1);
end
...
colormap('jet');cb=colorbar;cb.Position = cb.Position.*[1.07,1,1,1];
caxis([0,1013250]);
axis equal; axis off;ax=axis;axis(ax*1.001); view([0,90]);
hold on; plot3(FractureData(:,[1,3])',FractureData(:,[2,4])',2000000*ones(size(FractureData(:,[2,4])')),...
...
ParametersofSlice(:,8:11)=([0,500,700,500; 625,0,625,600]-300)*(1-exp(-7))+300;
...
x=xy(:,1);
y=xy(:,2);
k_m11 = ones(size(x))*1e-14;
end
function k_m12 = k_m12(xy)
x=xy(:,1);
y=xy(:,2);
k_m12 = zeros(size(x))*1e-14;
end
function k_m21 = k_m21(xy)
x=xy(:,1);
y=xy(:,2);
k_m21 = zeros(size(x))*1e-14;
end
function k_m22 = k_m22(xy)
x=xy(:,1);
y=xy(:,2);
k_m22 = ones(size(x))*1e-14;
end
